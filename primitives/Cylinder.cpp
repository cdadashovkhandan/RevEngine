...
Cylinder::Cylinder() {
    shapeType = PrimitiveType::CYLINDER;
}

Cylinder::~Cylinder() {}

std::vector<ParamPair> Cylinder::buildParameters(std::vector<pcl::PointXYZ, Eigen::aligned_allocator<pcl::PointXYZ>> const points, float maxMagnitude) const
{
...
bool Cylinder::isIntersecting(pcl::PointXYZ const point, std::vector<float> const params, float const maxMagnitude) const
{
...
/**
 * @brief Cylinder::calculateMFE Calculate Mean Fitting error for a given set of points and their distances from estimates.
 * @param points
 * @param distances
 * @return
 */
float Cylinder::calculateMFE(pcl::PointCloud<pcl::PointXYZ>::Ptr const cloud)
{
    /*
    Matlab equivalent:
...
std::shared_ptr<RenderShape> Cylinder::getRenderShape() const
{
...
std::vector<Eigen::Vector3f> Cylinder::getBaseVertices() const
{
    std::vector<Eigen::Vector3f> vertices;

    // TODO: Load vertices from a file

    return vertices;
}

//TODO:
// Eigen::Vector3f Cylinder::getNormal() const
// {
//
